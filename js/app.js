...
const BUILD = '5 · grouped grocery list';
...
function buildSelectedGroceryList(keepScroll) {
  ...
  const body = aisles.map(a => `
    <section class="aisle">
      ...
      <ul>${a.items.map(i => `
        <li><label><input type="checkbox"><span class="tick"></span><span class="what">${i.amount ? `<b>${i.amount}</b> ` : ''}${i.label}${showGrocerySources ? `<small>${i.recipes.join(' · ')}</small>` : ''}</span></label></li>`).join('')}
      </ul>
    </section>`).join('');
